...
- **Implementation:**
  ...
  function isValidDrawDTO(obj: any): obj is DrawDTO {
    return typeof obj.id === "string" && typeof obj.name === "string" && typeof obj.created_at === "string";
  }

  const isValidResponse = Array.isArray(data) && data.every(isValidDrawDTO);
  ```
...
- **Implementation:**
  ...
  const updateShadows = (e: ScrollEvent) => {
    ...
    const hasRightShadow = target.scrollLeft < target.scrollWidth - target.clientWidth;
